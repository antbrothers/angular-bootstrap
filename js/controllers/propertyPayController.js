 ...
 * Date:2016/8/22
 */
var myController=angular.module('propertyPay',[]);
myController.controller('propertyPayCtrl',["$scope","propertyPayService","$timeout","localStorageService","$location","authorize",function($scope,propertyPayService,$timeout,localStorageService,$location,authorize){
   console.log("产权缴费");
   ...
   if($scope.openId =="" || $scope.openId==null || $scope.customerId=="" || $scope.customerId==null){
     /* $scope.encodeuri=encodeURI("http://p-share.cn/wx_share/wxpay/getAuthor?backUri=http://p-share.cn/wx_share/wxpay/getCode?directUrl=http://p-share.cn/wx_share/html5/views/index.html%23/home/login&type=propertyPay");*/
      var p = authorize.path();
      $scope.encodeuri=encodeURI(""+p+"login&type=propertyPay");
   ...
   $scope.licensePlate = function(){
      propertyPayService.getProvince().success(function(req){
         $('.jianpan').show();
         num = 0;
   ...
   $scope.EnglishNumber = function(){
      propertyPayService.getEnglishNumber().success(function(req){
         $('.jianpan').show();
         num = 0;
         getNum=angular.element('.currnt').attr('data-val');
...
//		$scope.data.num6=angular.element($event.currentTarget).text();
   ...
   $scope.clear = function($event){
      ...
      indexId = numAll--;
	  	...
	  	$scope.parkName="";
        $scope.parkLists="";
        $scope.effectEndTime ="";                  // 产权 当前有效时间
        $scope.price = "";                         // 产权 单价
        $scope.totalPrice = "";                    // 产权 订单金额
        $scope.monthJf = "1";
        $scope.effectEndTime="";
        $scope.addValidTime="";
   ...
   $scope.getPay=function(parkingId,customerId,carNumber){
      propertyPayService.getOrderData(parkingId,customerId,carNumber).success(function(req){
   ...
   $scope.getCarList=function(carNumber){
      ...
      propertyPayService.getDataByScanOrMenu($scope.customerId,carNumber).success(function(req){
         $.helpTool().loading().close();
         if(req.errorNum=='0'){
            if(req.data.length==1){
               ...
               layer.open({
                  content: '<h5 style="font-weight: bold">提示</h5>当前车辆在多个车场有产权订单,请选择正确车场查询',
...
//            var carNumber_val = angular.element('.carNumber').val();
        ...
        /* var beginDate = "2016/01/01";
         ...
         $.helpTool().loading().open();
         propertyPayService.orderc($scope.parkingId, $scope.customerId, $scope.plate,14, $scope.effectEndTime, $scope.monthJf).success(function (req) {
            $.helpTool().loading().close();
            if (req.errorNum == "0") {
               ...
               //获取授权配置参数
               propertyPayService.getWxConfig().success(function (data) {
                  wx.config({
                     /* debug: true, // 开启调试模式*/
                     appId: data.appId,
                        ...
                        success: function (data) {
                           wx.chooseWXPay({
                              ...
                              success: function () {
                                 ...
                                 window.location.href = "http://p-share.cn/wx_share/html5/views/index.html#/home/paySuccess?orderId=" + req.order.orderId + "&orderType=" + 14;
                              },
                              fail: function () {
                              }
                           });
