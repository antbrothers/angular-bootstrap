...
'use strict';
var myService=angular.module('monthPayServices',[]);
myService.factory('monthService',['$http',function($http){
    return{
        //车场列表
        getDataByScanOrMenu:function(customerId,carNumber){
            return $http.get("/wx_share/weixinPayment/getDataByScanOrMenu?orderType=13"+"&customerId="+customerId+"&carNumber="+carNumber);
        },
        //获取缴费数据
        getOrderData:function(parkingId,customerId,carNumber){
            return $http.get("/wx_share/weixinPayment/getOrderData?parkingId="+parkingId+"&orderType=13"+"&customerId="+customerId+"&carNumber="+carNumber);
        },
        /*获取键盘内容*/
        //获取省简称
